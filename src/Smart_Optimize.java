...
class Smart_Optimize {
	...
	HashMap<String, ArrayList<Double>> investing_helper(Scientist sci) {
		...
		for (int i = 1; i<=b.num_paths; i++) {
			...
			if (branch.isEmpty()) {continue;} // skip initial case where scientist has not learn any ideas and doesn't learn any new ideas
	...
	double get_V(ArrayList<Integer> branch, Scientist sci) {
		...
		int k = 10; // how many chunks of effort
		...
		Generator<HashMap<Integer, Double>> effort_perm_gen = new Generator<>() {
			public void run() throws InterruptedException {
				...
				int j = k; int j_1 = k; int q = k; // init here for scoping
				int r = objects.length - 1;
				int[] a = new int[k]; // initial multiset indexes, automatically fills array of 0's with length k
				while (true) {
					int[] out = IntStream.range(0, k-1).map(i -> objects[a[i]]).toArray(); // in python words, [objects[a[i]] for i in range(0,k)] --> stores index of element in arraylist branch
